...
- **Not a deploy permalink.** `https://6a7f9b1f91786e00086f40d4--your-site.netlify.app` is the address Netlify shows on a deploy's page, and it is a bare origin with no path — so it passes the rule above and is still wrong. It is a *different host* from your site, and the PKCE verifier cookie set when the link was requested is host-only (`netlify.app` is a public suffix, so it can't span both). Every magic link then reports itself as **expired or already used** when nothing has expired. `src/lib/deploy-origin.ts` bounces such a link back to the real site and logs it, but the setting is the actual fix.
