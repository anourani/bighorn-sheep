...
- **Note what the preview entry also matches.** `https://**--your-site.netlify.app/**` covers deploy previews, and equally covers `https://<deploy-id>--your-site.netlify.app` — the permanent per-deploy address on Netlify's deploy page. Since `emailRedirectTo` comes from `window.location.origin`, anyone who opens `/login` on a permalink gets a magic link back to that permalink, Supabase accepts it, and they sign in against a frozen old build with no error anywhere. `src/middleware.ts` redirects `/login` off a permalink for that reason; previews and branch deploys are exempt, being their own origins by design.
