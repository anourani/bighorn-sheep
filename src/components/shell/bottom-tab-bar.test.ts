...
describe("the tab bar's selected-tab token", () => {
  it("is defined in the Tailwind config at the design's hex", async () => {
    expect(
      await read(CONFIG),
      "colors.fill.deep is the #EAEAEA fill Figma gives the selected tab.",
    ).toContain('deep: "#EAEAEA"');
  });

  it("is the token the bar actually reaches for", async () => {
    ...
    expect(await read(BAR)).toContain("bg-fill-deep");
  });
});
...
describe("the tab bar's height", () => {
  it("is declared once, in globals.css", async () => {
    expect(
      await read(CSS),
      "--tab-bar-h is 4 + 56 + 4, off the Figma frame. It lives on :root because " +
        "Toast portals to document.body and inherits nothing from the app shell.",
    ).toContain("--tab-bar-h: 64px");
  });
  ...
  it("is read, not retyped, by both consumers", async () => {
    ...
    expect(await read(BAR)).toContain("h-[var(--tab-bar-h)]");
