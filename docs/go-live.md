...
- [ ] The **Layout** filter reads *Matchups*, and the week shows as fixtures — one card
      per game, home team over away.
- [ ] Switching Layout to *Grid* shows **32 cards** — every team, with byes marked
      *BYE Week* and spent teams *Already Selected*. A short grid means the schedule
      didn't fully load. A pick made in one layout is already selected in the other.
